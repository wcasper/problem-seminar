...
\pause
Therefore $r_1r_2r_3r_4 = \frac{9}{5}$.
\end{soln}
\end{frame}
...
$$f(x)  = 5(x-r_1)(x-r_2)(x-r_3)(x-r_4)$$
\pause
$$f(-2)  = 5(-2-r_1)(-2-r_2)(-2-r_3)(-2-r_4)$$
\pause
$$\frac{5}{f(-2)}  = \frac{1}{r_1+2}\frac{1}{r_2+2}\frac{1}{r_3+2}\frac{1}{r_4+2} = \frac{5}{151}$$
\end{solution}
\end{frame}
...
\begin{itemize}
\item make $0$ a root
\item make other roots come in $\pm$ pairs: 
\end{itemize}
$$f(x) = x(x-a)(x+a)(x-b)(x+b) = x(x^2-a^2)(x^2-b^2) = x^5 - (a^2+b^2)x^4 + a^2b^2x$$
\end{soln}
\end{frame}

\begin{frame}{Putnam 1985 B1}
\begin{soln}
Take $a,b$ to be integers.\\
$$m_3 = -m_2 = a,\quad a_1=-a_4=b$$
$$f(x) = x^5 - (a^2+b^2)x^4 + a^2b^2x$$
\pause
\textbf{Suspicion:} the answer is we can make two coefficients zero.\\
\pause
...
\begin{itemize}
\pause
\item Just one nonzero coefficient?\\
\pause
$$f(x) = x^5\quad\text{has repeated roots...impossible!}$$
\pause
\item Just two nonzero coefficients?\\
\pause
$$f(x) = x^5+ax^n,\ n\geq 2\quad\text{has repeated roots...impossible!}$$
\pause
